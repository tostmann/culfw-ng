...
*   **[IN PROGRESS]** **Build-Prozess für `serial`-Profil finalisieren:** Der Build scheitert nur noch am finalen Linker-Fehler `undefined reference to 'usb_serial_jtag_write_bytes'`. Obwohl die Komponente in der `CMakeLists.txt` deklariert ist, wird sie im Minimal-Build nicht gelinkt. Die Vermutung ist eine fehlende Kconfig-Option in `sdkconfig.defaults.serial`, die den Treiber selbst aktiviert.

## 4. Erkenntnisse & Gelöste Probleme

*   **Hypothese (Unverifiziert): WiFi-Interferenz stört CC1101-Empfang.**
    *   **Analyse:** Trotz korrigierter GDO-Pins und verifiziertem RX-Modus des CC1101 wird nur Rauschen empfangen, aber keine Signale eines nahen Senders dekodiert. Es besteht der Verdacht, dass der aktive 2.4-GHz-WiFi-Funk des ESP32-C6 den empfindlichen 433-MHz-Empfänger des CC1101 stört (Jamming). Die Verifizierung dieser Hypothese ist aktuell durch Build-Probleme blockiert.
...
*   **[BLOCKIERT]** **Build-Prozess für `serial`-Profil finalisieren (Höchste Priorität):** Behebung des letzten verbleibenden Linker-Fehlers `undefined reference to 'usb_serial_jtag_write_bytes'`. Dies erfordert die Identifizierung und Aktivierung der korrekten Kconfig-Optionen in `sdkconfig.defaults.serial`, um den USB-Serial-JTAG-Treiber im Minimal-Build zu aktivieren und zu verlinken.
*   **Validierung der RF-Empfangsstabilität:** Nach erfolgreichem `serial`-Build, Testen der Firmware ohne Netzwerkstacks, um die Hypothese der 2,4-GHz-WiFi-Interferenz auf den 433-MHz-Empfang zu verifizieren. Ziel ist der saubere Empfang und die Dekodierung von Signalen eines Legacy-CULs.
