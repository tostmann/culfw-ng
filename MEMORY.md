...
*   **[DONE]** **Build-System gehärtet:** Eine zentrale Header-Datei (`main.h`) wurde eingeführt, um Feature-Makros (`APP_MATTER_ENABLED`) konsistent zu definieren. Das Build-Skript wurde angepasst, um für das `serial`-Profil eine saubere, isolierte `sdkconfig` zu erstellen.
*   **[IN PROGRESS]** **Build-Prozess für `serial`-Profil finalisieren:** Die Entkopplung vom Matter-SDK erweist sich als komplex. Aktuell scheitert der Build an Linker-Fehlern, da Basistreiber (wie `usb_serial_jtag`) und die `matter_bridge`-Aufrufe trotz Makro-Absicherung nicht korrekt aufgelöst werden. Die `CMakeLists.txt` und die Preprozessor-Logik müssen weiter verfeinert werden, um einen echten netzwerkfreien Build zu ermöglichen.
...
*   **Erkenntnis: Extreme Kopplung durch ESP-Matter SDK.**
    ...
    *   **Konsequenz:** Eine mehrstufige Entkopplungsstrategie ist notwendig: (1) Bedingte Kompilierung von Quellcode-Dateien in `CMakeLists.txt`, (2) Absicherung aller Cross-Component-Funktionsaufrufe mit Präprozessor-Makros und (3) explizite Deklaration aller benötigten Basis-IDF-Komponenten in der `CMakeLists.txt`, um unabhängig von Matter lauffähig zu sein. Dieser Prozess ist noch nicht abgeschlossen.
...
*   **[BLOCKIERT]** **Build-Prozess für `serial`-Profil finalisieren (Höchste Priorität):** Behebung der verbleibenden Linker-Fehler (`undefined reference to 'usb_serial_jtag_write_bytes'` und `matter_bridge_report_event`). Dies erfordert eine finale Korrektur der `CMakeLists.txt` zur korrekten Einbindung der IDF-Basistreiber und eine Überprüfung der Preprozessor-Logik (`APP_MATTER_ENABLED`), um sicherzustellen, dass Matter-bezogener Code vollständig aus dem Kompilat entfernt wird.
