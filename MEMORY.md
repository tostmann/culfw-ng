...
*   **Software-Architektur:** FreeRTOS-Task-basiert.
    ...
    *   `slowrf_task`: Implementiert eine Zustandsmaschine (Sync-Erkennung -> Bit-Akkumulation -> Paritätsprüfung), um aus den vom CC1101 empfangenen Pulsfolgen gültige Datenpakete zu dekodieren. Das FS20-Protokoll erfordert eine **ungerade Parität (Odd Parity)**.
...
*   **Signal-Aussendung (TX):**
    ...
    *   Für eine korrekte ASK-Modulation wird die `PATABLE` mit zwei Werten (`{0x00, 0xC0}`) initialisiert, um "0" (keine Leistung) und "1" (maximale Leistung) klar zu definieren.
    *   Pakete werden zur Erhöhung der Übertragungssicherheit mehrfach (3x) gesendet.
...
*   **[DONE]** Basis-Implementierung des CC1101 SPI-Treibers (`cc1101.c`), inklusive Burst-Write-Funktion.
...
*   **[DONE]** Erweiterter SlowRF-Empfang (RX) mit Zustandsmaschine, der erfolgreich FS20-Pakete dekodiert.
*   **[DONE]** SlowRF-Senden (TX) implementiert, inklusive Paket-Wiederholung und korrekter ASK-Modulation via `PATABLE`.
*   **[DONE]** Test-Funktion (`Tr`) zum Senden von zufälligen, validen FS20-Paketen implementiert.
*   **[DONE]** FS20-Paritätsprüfung (Odd Parity) im TX-Encoder und RX-Decoder implementiert.
...
*   **[DONE]** Kalibrierung des RX-Decoders: Timing-Toleranzen für FS20 sind justiert und funktional.
*   **[DONE]** Validierung der TX-Kette: Die grundlegenden TX-Fehler (Modulation, Parität) wurden behoben.

## 4. Neue Erkenntnisse / Probleme

*   **[INFO]** Das Auslesen der CC1101-Statusregister (z.B. PARTNUM) erfordert das Setzen des Burst-Read-Bits, auch für einzelne Bytes, um eine Fehlinterpretation als Command-Strobe zu vermeiden.
*   **[INFO]** Eine korrekte ASK/OOK-Modulation auf dem CC1101 erfordert die Initialisierung der `PATABLE` mit zwei Werten (`{0x00, 0xC0}`), um die Sendeleistung für logisch '0' (keine Leistung) und '1' (volle Leistung) festzulegen.
...
*   **[INFO]** Das `MARCSTATE`-Register (`0x35`) ist ein essenzielles Diagnosewerkzeug, um sicherzustellen, dass sich der CC1101 im korrekten Zustand befindet (z.B. `0x0D` für RX).
...
*   **End-to-End Test:** Validierung der Firmware mit einem Host-System (z.B. FHEM) zur Sicherstellung der Kompatibilität und Langzeitstabilität.
