...
*   **[DONE]** **Build-Prozess für `serial`-Profil finalisiert:** Der Build-Prozess wurde grundlegend gehärtet und stabilisiert. Der Linker-Fehler (`undefined reference to 'usb_serial_jtag_write_bytes'`) wurde durch eine 3-stufige Strategie behoben: (1) Das Build-Skript (`build_idf.sh`) wurde korrigiert, um die korrekte `sdkconfig.defaults.serial` und das Ziel-Target (`-DIDF_TARGET=esp32c6`) explizit an `idf.py` zu übergeben. (2) Die `sdkconfig.defaults.serial` wurde um die notwendigen Treiber-Optionen für USB-JTAG erweitert. (3) Der C-Code (`culfw_parser.c`, `main.c`) wurde mittels Präprozessor-Direktiven (`#ifndef PROFILE_SERIAL`) von Netzwerk- und Matter-Abhängigkeiten entkoppelt, um einen sauberen, minimalen Build zu ermöglichen.
...
*   **Gelöstes Problem: Linker-Fehler und Build-Instabilität im 'serial'-Profil.**
    *   **Analyse:** Der Build scheiterte an einer Kombination von drei Problemen: (1) Das Build-Skript hat die `sdkconfig`-Datei für das `serial`-Profil nicht zuverlässig an `idf.py` übergeben. (2) Das Build-System fiel auf die falsche Zielarchitektur (Xtensa/ESP32 statt RISC-V/ESP32-C6) zurück. (3) Der C-Code hatte feste Abhängigkeiten zu Netzwerk-Funktionen, die im `serial`-Profil nicht vorhanden waren und zu "undefined reference"-Fehlern führten.
    *   **Lösung:** Eine mehrschichtige Korrektur: (1) Das `build_idf.sh`-Skript wurde gehärtet und übergibt nun explizit `-DSDKCONFIG_DEFAULTS=...` und `-DIDF_TARGET=esp32c6`. (2) Harte Coded-Abhängigkeiten in `culfw_parser.c` und `main.c` wurden durch `#ifndef PROFILE_SERIAL`-Präprozessor-Direktiven entfernt, um eine saubere Kompilierung ohne Netzwerk-Stack zu gewährleisten.
...
*   **Validierung der RF-Empfangsstabilität (Höchste Priorität):** Testen der nun erfolgreich kompilierten `serial`-Firmware ohne Netzwerkstacks. Verifizierung der Hypothese der 2,4-GHz-WiFi-Interferenz auf den 433-MHz-Empfang. Ziel ist der saubere Empfang und die Dekodierung von Signalen eines Legacy-CULs.
