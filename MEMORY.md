...
*   **[DONE]** **Build-System (Serial-Profil):** Die Isolation des `serial`-Profils vom Matter-SDK wurde erfolgreich umgesetzt. Durch eine Kombination aus einer zentralen Header-Datei (`main.h`), expliziten Compiler-Definitionen (`-DPROFILE_SERIAL=1`) und einer strikt getrennten `sdkconfig.defaults.serial` werden Matter-bezogener Code und Netzwerk-Quellen nun zuverlässig aus dem Build ausgeschlossen. Der Linker-Fehler `undefined reference to 'matter_bridge_report_event'` ist behoben.
*   **[IN PROGRESS]** **Build-Prozess für `serial`-Profil finalisieren:** Der Linker-Fehler (`undefined reference to 'usb_serial_jtag_write_bytes'`) ist nach einem `clean` wieder aufgetreten. Die Ursache wurde identifiziert: Das `build_idf.sh`-Skript wendet die `sdkconfig.defaults.serial` nicht korrekt an. Die Behebung steht aus.

## 4. Erkenntnisse & Offene Probleme

*   **Offenes Problem: Linker-Fehler `usb_serial_jtag_write_bytes` im `serial`-Profil.**
    *   **Analyse:** Obwohl die `MEMORY.md` den Fehler als behoben markierte, ist er nach einem `clean` des Build-Verzeichnisses wieder aufgetreten. Dies deutet darauf hin, dass die Lösung (die Aktivierung der korrekten Kconfig-Optionen) nur in einer lokalen, nicht versionierten `sdkconfig`-Datei existierte und nie in die `sdkconfig.defaults.serial` übernommen wurde. Obwohl diese Optionen nun in `sdkconfig.defaults.serial` eingetragen sind, werden sie vom aktuellen `build_idf.sh`-Skript nicht korrekt an `idf.py` übergeben, weshalb die finale Konfiguration unvollständig ist und der Treiber nicht gelinkt wird.
    *   **Nächster Schritt:** Das `build_idf.sh` Skript muss korrigiert werden, um sicherzustellen, dass die `sdkconfig`-Datei für den `serial`-Build sauber und ausschließlich aus `sdkconfig.defaults.serial` generiert wird.
*   **Hypothese (Unverifiziert): WiFi-Interferenz stört CC1101-Empfang.**
    *   **Analyse:** Trotz korrigierter GDO-Pins und verifiziertem RX-Modus des CC1101 wird nur Rauschen empfangen, aber keine Signale eines nahen Senders dekodiert. Es besteht der Verdacht, dass der aktive 2.4-GHz-WiFi-Funk des ESP32-C6 den empfindlichen 433-MHz-Empfänger des CC1101 stört (Jamming). Die Verifizierung dieser Hypothese ist möglich, sobald der `serial`-Build erfolgreich ist.
*   **Gelöstes Problem: Extreme Kopplung durch ESP-Matter SDK.**
    *   **Analyse:** Der Versuch, einen minimalen `serial`-Build ohne Netzwerkstacks zu erstellen, scheiterte wiederholt an tiefgreifenden Abhängigkeiten. Das `esp-matter` SDK als "managed component" zwingt dem Build-System transitive Abhängigkeiten (z.B. WiFi) auf und überschreibt explizit gesetzte `sdkconfig`-Werte, was die Entkopplung extrem erschwert.
    *   **Lösung:** Die Entkopplung wurde durch eine mehrstufige Strategie erfolgreich umgesetzt: (1) Eine zentrale Header-Datei (`main.h`) definiert ein exklusives `APP_MATTER_ENABLED` Makro, das von einem via Build-Skript gesetzten `-DPROFILE_SERIAL=1` gesteuert wird. (2) Die `CMakeLists.txt` nutzt dieses Profil-Makro, um Quellcode-Dateien und Abhängigkeiten bedingt einzubinden. (3) Das Build-Skript (`build_idf.sh`) wurde gehärtet, um für jedes Profil eine saubere, isolierte `sdkconfig`-Datei zu verwenden. Diese Kombination umgeht das Problem, dass das Matter-SDK die Kconfig-Einstellungen überschreibt und löste die Matter-bezogenen Linker-Fehler.
*   **Gelöstes Problem: Laufzeit-Crash (`modem_clock`) bei simultaner Aktivierung von WiFi und Thread.**
    ...
    *   **Lösung:** Eine grundlegende architektonische Umstellung auf **dedizierte, sich gegenseitig ausschließende Build-Profile**. Es wurden drei Konfigurationen (`sdkconfig.defaults.wifi`, `sdkconfig.defaults.thread`, `sdkconfig.defaults.serial`) erstellt, die sicherstellen, dass nur ein Netzwerkstack zur Zeit einkompiliert wird.
*   **Gelöstes Problem: Kein Empfang von RF-Signalen trotz korrekter Frequenz.**
    *   **Analyse:** Der CC1101-Treiber (`cc1101.c`) hatte die Registerkonfiguration für `GDO0` (Daten-Interrupt-Pin) und `GDO2` (Carrier Sense Pin) vertauscht. Die Interrupt Service Routine wartete auf Pulse am `GDO0`-Pin, dieser war jedoch als Carrier-Sense konfiguriert.
    *   **Lösung:** Die Werte für die Register `IOCFG0` und `IOCFG2` wurden getauscht, sodass `GDO0` nun korrekterweise als "Serial Data Output" fungiert. Dies hat den Empfang von rohen Pulsfolgen freigeschaltet.
...
*   **Behebung des Build-Prozesses für `serial`-Profil (Höchste Priorität):** Anpassung des `build_idf.sh`-Skripts, um die `sdkconfig.defaults.serial` korrekt anzuwenden und den Linker-Fehler `undefined reference to 'usb_serial_jtag_write_bytes'` endgültig zu beheben.
*   **Validierung der RF-Empfangsstabilität:** Sobald der `serial`-Build erfolgreich ist, Testen der Firmware ohne Netzwerkstacks, um die Hypothese der 2,4-GHz-WiFi-Interferenz auf den 433-MHz-Empfang zu verifizieren. Ziel ist der saubere Empfang und die Dekodierung von Signalen eines Legacy-CULs.
...
*   **Build-System:** Umgestellt auf natives **ESP-IDF (`idf.py`)**. Das Build-Skript (`build_idf.sh`) unterstützt **dedizierte Build-Profile (`wifi`, `thread`, `serial`)**. Die Isolation wird durch drei Mechanismen sichergestellt: (1) Jedes Profil hat eine eigene, exklusive `sdkconfig.defaults.<profil>`-Datei. (2) Das Skript übergibt eine eindeutige Compiler-Definition (z.B. `-DPROFILE_SERIAL=1`) an CMake. (3) Die `CMakeLists.txt` und der C-Code (`main.h`) nutzen diese Definition, um Quellcode, Abhängigkeiten und Feature-Makros (`APP_MATTER_ENABLED`) zur Kompilierzeit präzise zu steuern.
