...
*   **[DONE]** **Build-System gehärtet:** Eine zentrale Header-Datei (`main/main.h`) wurde eingeführt, um Feature-Makros (`APP_MATTER_ENABLED`) konsistent zu definieren. Das Build-Skript wurde angepasst, um für das `serial`-Profil eine saubere, isolierte `sdkconfig` zu erstellen.
*   **[IN PROGRESS]** **Build-Prozess für `serial`-Profil finalisieren:** Die Entkopplung vom Matter-SDK erweist sich als komplex. Aktuell scheitert der Build an Linker-Fehlern (`undefined reference to 'usb_serial_jtag_write_bytes'` und `matter_bridge_report_event`), da Basistreiber und die `matter_bridge`-Aufrufe trotz Makro-Absicherung und korrekter `CMakeLists.txt`-Deklarationen nicht korrekt aufgelöst werden. Die Interaktion zwischen `sdkconfig`, CMake und den Preprozessor-Makros muss weiter analysiert werden.
...
*   **Erkenntnis: Extreme Kopplung durch ESP-Matter SDK.**
    *   **Analyse:** Der Versuch, einen minimalen `serial`-Build zu erstellen, scheiterte wiederholt an tiefgreifenden Abhängigkeiten. Das `esp-matter` SDK als "managed component" wird standardmäßig in den Build-Prozess integriert und verursacht Kompilierungs- und Linker-Fehler, selbst wenn es in der Applikation nicht aktiv genutzt wird. Die transitive Einbindung von IDF-Basiskomponenten (wie `esp_driver_usb_serial_jtag`) durch Matter verschleiert die tatsächlichen Abhängigkeiten der eigenen Applikation und erschwert die Entkopplung erheblich.
    *   **Konsequenz:** Eine mehrstufige Entkopplungsstrategie ist notwendig: (1) Bedingte Kompilierung von Quellcode-Dateien in `CMakeLists.txt`, (2) Absicherung aller Cross-Component-Funktionsaufrufe mit Präprozessor-Makros in einer zentralen Header-Datei (`main.h`) und (3) explizite Deklaration aller benötigten Basis-IDF-Komponenten in der `CMakeLists.txt`. Trotz Umsetzung dieser Maßnahmen bleiben Linker-Fehler bestehen, was auf subtile Probleme in der Build-Toolchain hindeutet.
...
*   **[BLOCKIERT]** **Build-Prozess für `serial`-Profil finalisieren (Höchste Priorität):** Behebung der verbleibenden Linker-Fehler (`undefined reference to 'usb_serial_jtag_write_bytes'` und `matter_bridge_report_event`). Dies erfordert eine finale Korrektur der `CMakeLists.txt` und der Preprozessor-Logik (`APP_MATTER_ENABLED`), um sicherzustellen, dass Matter-bezogener Code vollständig aus dem Kompilat entfernt und die IDF-Basistreiber korrekt eingebunden werden.
