...
*   **[DONE]** **Build-Prozess für `serial`-Profil finalisieren:** Der letzte verbleibende Linker-Fehler (`undefined reference to 'usb_serial_jtag_write_bytes'`) wurde durch die Aktivierung der korrekten Kconfig-Optionen in `sdkconfig.defaults.serial` behoben.

## 4. Erkenntnisse & Gelöste Probleme

*   **Hypothese (Unverifiziert): WiFi-Interferenz stört CC1101-Empfang.**
    *   **Analyse:** Trotz korrigierter GDO-Pins und verifiziertem RX-Modus des CC1101 wird nur Rauschen empfangen, aber keine Signale eines nahen Senders dekodiert. Es besteht der Verdacht, dass der aktive 2.4-GHz-WiFi-Funk des ESP32-C6 den empfindlichen 433-MHz-Empfänger des CC1101 stört (Jamming). Die Verifizierung dieser Hypothese ist nun möglich.
*   **Gelöstes Problem: Linker-Fehler `usb_serial_jtag_write_bytes` im Minimal-Build.**
    *   **Analyse:** Der Build des `serial`-Profils schlug fehl, weil die `usb_serial_jtag_write_bytes`-Funktion nicht gefunden wurde. Obwohl die Komponente `esp_driver_usb_serial_jtag` in der `CMakeLists.txt` deklariert war, war der Treiber selbst im SDK nicht aktiviert.
    *   **Lösung:** Ein Vergleich mit der funktionierenden `sdkconfig` des `v1.1.0-NG` Releases zeigte, dass mehrere `Kconfig`-Optionen fehlten. Durch Hinzufügen von `CONFIG_USJ_ENABLE_USB_SERIAL_JTAG=y` und `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG_ENABLED=y` zu `sdkconfig.defaults.serial` wurde der Treiber korrekt aktiviert und gelinkt, was den Build-Fehler behob.
...
*   **Validierung der RF-Empfangsstabilität (Höchste Priorität):** Mit dem nun erfolgreichen `serial`-Build, Testen der Firmware ohne Netzwerkstacks, um die Hypothese der 2,4-GHz-WiFi-Interferenz auf den 433-MHz-Empfang zu verifizieren. Ziel ist der saubere Empfang und die Dekodierung von Signalen eines Legacy-CULs.
